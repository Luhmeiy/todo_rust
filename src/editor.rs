...
fn complete_from<T: AsRef<str>>(items: &[T], line: &str, prefix: &str) -> Vec<Suggestion> {
    let add_prefix = needs_prefix(line, prefix);
    ...
    items
        ...
        .map(|item| {
            ...
            Suggestion {
                ...
                span: calculate_span(line, prefix),
                ..Default::default()
            }
        })
        .collect()
...
impl TodoCompleter {
    ...
    fn complete_command(&self, cmd: &str, line: &str, prefix: &str) -> Vec<Suggestion> {
        ...
        if !subcommands.is_empty() {
            complete_from(&subcommands, line, prefix)
        } else if !flags.is_empty() {
            complete_from(flags, line, prefix)
        } else if matches!(cmd, "save" | "load") {
            complete_from(&self.aliases, line, prefix)
        } else if cmd == "help" {
            complete_from(&self.commands, line, prefix)
        } else {
            vec![]
        }
    }
...
impl Completer for TodoCompleter {
    fn complete(&mut self, line: &str, _pos: usize) -> Vec<Suggestion> {
        ...
        if parts.is_empty() {
            if self.list_exists {
                return complete_from(&self.commands, line, "");
            } else {
                return complete_from(&self.allowed_empty, line, "");
            }
        }

        if !is_command(parts[0]) {
            return complete_from(&self.commands, line, parts[0]);
        }

        match parts.len() {
            1 => self.complete_command(parts[0], line, ""),
            _ => {
                if is_already_used(parts[0], parts[1]) {
                    return vec![];
                }
                ...
                if is_command(parts[1]) {
                    if parts.len() > 2 && is_already_used(parts[1], parts[2]) {
                        return vec![];
                    }
